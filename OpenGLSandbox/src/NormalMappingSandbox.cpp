...
NormalMappingSandbox::NormalMappingSandbox()
...
void NormalMappingSandbox::OnAttach()
{
    EnableGLDebugging();
    ...
    GenerateTexture2D("assets/textures/brickwall/brickwall-normal.jpg", &m_BrickwallNormal, GL_REPEAT, GL_LINEAR, true);
...
void NormalMappingSandbox::OnDetach()
{
    // TODO
...
void NormalMappingSandbox::OnEvent(OpenGLCore::Event& event)
{
    EventDispatcher dispatcher(event);
    dispatcher.Dispatch<WindowResizeEvent>(BIND_EVENT_FN(NormalMappingSandbox::OnWindowResized));
}

void NormalMappingSandbox::OnUpdate(OpenGLCore::Timestep ts)
{
    // Update
    ...
    model = glm::rotate(model, glm::radians((float)glfwGetTime() * -8.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0)));
...
bool NormalMappingSandbox::OnWindowResized(WindowResizeEvent& event)
{
